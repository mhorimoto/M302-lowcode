...
void setup(void) {
    ...
    sht4x.softReset();
    delay(100);
    if (sht4x.serialNumber(serialNumber)==NO_ERROR) {
        sht4x_present = true;
        Serial.print("SHT4x SN:");
...
void UserEvery10Seconds(void) {
    ...
    if (sht4x_present) {
        ...
        Serial.print("SHT4x Temp:");
        Serial.print(aTemperature);
        Serial.print(" Humid:");
        Serial.println(aHumidity);
        if (error == NO_ERROR) {
            dtostrf(aTemperature,0,1,dval);
            dtostrf(aHumidity,0,0,wval);
        } else {
            sprintf(dval,"ERR");
            sprintf(wval,"ERR");
        }
        sprintf(lcdtext[2],"%sC SHT %s%%",dval,wval);
    }
    if (ds.select(dry_address)) {
        dryv = ds.getTempC();
