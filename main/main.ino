...
//  - ADT7410 (Akizuki) for temperature measurement (func:6)
...
unsigned long cndValue = 0;       // CCM cnd Value
const unsigned long interval = 1000; // 1秒（1000ms）間隔

unsigned int sensorFlags = 0; // ビットフラグでセンサーの状態を管理
#define SENSOR_SHT4X 0x01
#define SENSOR_ADT7410 0x02
#define SENSOR_ADS1115 0x04
...
void setup(void) {
    ...
    //
    //**********************************
    sensorFlags = 0; // Reset sensor flags
    ...
    if (sht4x.softReset() != 0) {
        ...
        sensorFlags &= ~SENSOR_SHT4X; // SHT4X not found
    } else {
        sensorFlags |= SENSOR_SHT4X;
    }
    adt7410.begin(ADT7410_I2CADDR_DEFAULT);
    delay(200);
    if (!adt7410.reset()) {
        for(i=0;i<3;i++) {
            uecsSendData(0, xmlDT, "0x20000200", 0); // NO ADT7410 cnd
            recv16528port();
            digitalWrite(LED1, HIGH);
            delay(100);
            digitalWrite(LED1, LOW);
...
void uecsSendData(int id, char *xmlDT, char *tval, int z) {
    ...
    if (EEPROM.read(a + LC_SEND_VALID) != 0x01)
        return;
...
void ope_ADT7410(int TempId) {
    char *xmlDT PROGMEM = CCMFMT;
    float aTemperature = 0.0;
    aTemperature = adt7410.readTempC();
